 ...
 if(d.kind==='cover'){
  ...
  if(d.kind==='explanation'){
   ...
   d.items.forEach(([heading,body],i)=>{
    const x=60+i*610;
    text(s,heading,x,213,550,70,28,true);
    text(s,body,x,294,550,240,27);
   });
   text(s,d.takeaway,60,567,1160,78,27,true);
  ...
  if(d.kind==='readerTable'){
   ...
   table(s,d.rows,d.widths,195,h,false);
   text(s,d.takeaway,60,565,1160,80,26,true);
   if(d.limit)text(s,d.limit,60,651,1150,45,18,false,'#384D64');
  }
  if(d.kind==='panels'){
  ...
  if(d.kind==='charts'){
   ...
   if(d.side)d.side.forEach(([label,body],i)=>{
    text(s,label,875,238+i*170,345,60,25,true);
    text(s,body,875,304+i*170,345,112,22);
   });
   text(s,d.takeaway,60,590,1160,64,26,true);
  }
  if(d.kind==='video') {
   text(s,d.videoGuide,60,107,1160,60,24);
   await picture(s,d.image,{left:220,top:173,width:820,height:461.25},'EMBEDDED_DEMO_VIDEO_POSTER');
  }
  if(d.kind==='table'){
