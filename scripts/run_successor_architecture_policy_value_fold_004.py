#!/usr/bin/env python3
"""Run the fresh deterministic-policy/value-fusion architecture successor."""
...
INSTRUMENT_ID = "successor-architecture-policy-value-fold-004-001"
INSTRUMENT_PATH = ROOT / (
    "research/05_evaluation/instruments/"
    "successor_architecture_policy_value_fold_004_001.json"
)
...
CONTROL_B = AutonomyArchitectureId.GOVERNED_SINGLE_PLANNER_B.value
CONTROL_C = AutonomyArchitectureId.HIERARCHICAL_MODEL_BASED_C.value
CANDIDATE_H = GuardedPolicyValuePlanner.implementation_id
CONDITIONS = (CONTROL_A, CONTROL_B, CONTROL_C, CANDIDATE_H)
...
def validate() -> dict[str, Any]:
    ...
    if instrument.get("instrument_id") != INSTRUMENT_ID:
        raise ArchitectureDevelopmentError("successor instrument identity drifted")
    public = _load_bound_package(instrument["dataset"], gold=False)
    ids = [str(row["case_id"]) for row in public["rows"]]
    if len(ids) != 150 or len(ids) != len(set(ids)):
        raise ArchitectureDevelopmentError("successor public case identity drifted")
    if tuple(instrument["conditions"]) != CONDITIONS:
        raise ArchitectureDevelopmentError("successor conditions drifted")
    if instrument["execution"]["maximum_provider_calls"] != 121:
        raise ArchitectureDevelopmentError("successor call ceiling drifted")
    if instrument["fixed_engine"]["provider_model"] != "gpt-5.6-luna":
        raise ArchitectureDevelopmentError("successor engine drifted")
    if not instrument["analysis"][
        "preferred_action_is_diagnostic_not_transition_validity"
    ]:
        raise ArchitectureDevelopmentError("successor validity definition drifted")
    return {
        "instrument_id": INSTRUMENT_ID,
        "status": "passed",
        "instrument_status": instrument["status"],
        "case_count": 150,
        "condition_cell_count": 600,
...
async def _planner_canary(
    ...
    response = await transport.call_with_ledger(
        ...
        provider_role="policy-value-planner-canary",
        system="Return one bounded synthetic pedagogical proposal.",
        prompt=_planner_prompt([row]),
        task="successor_policy_value_planner_canary",
    ...
    if proposal.selected_action not in eligible or any(
        ...
        raise ArchitectureDevelopmentError("planner canary left action envelope")
...
def _planner_for_condition(
    ...
    *,
    ...
    raise ArchitectureDevelopmentError(f"unknown condition: {condition_id}")
...
def _score(
    *,
    ...
    for response in responses:
        ...
        citation_valid = not intervention or (
            isinstance(payload, dict)
            and payload.get("source_range_keys") == [f"source-range-{case_id}"]
        )
        utility = float(gold["action_utilities"][selected])
        maximum_utility = max(float(value) for value in gold["action_utilities"].values())
        scored.append(
            {
                **response,
    ...
    for condition, rows in by_condition.items():
        ...
        aggregate[condition] = {
            ...
            / len(rows),
            "boundary_action_accuracy": sum(row["boundary_action_valid"] for row in rows)
            / len(rows),
            ...
            / len(rows),
            "p95_policy_regret": sorted(row["policy_regret"] for row in rows)[
            ...
            / len(rows),
            "citation_validity": sum(row["citation_valid"] for row in rows)
            / len(rows),
            "scope_validity": sum(row["scope_valid"] for row in rows) / len(rows),
        }

    a_rows = by_condition[CONTROL_A]
    c_rows = by_condition[CONTROL_C]
    h_rows = by_condition[CANDIDATE_H]
    h_vs_a_preference = _paired(
        h_rows,
        a_rows,
        value=lambda row: float(row["preferred_action_agreement"]),
        seed="fold-004-h-vs-a-preference",
    )
    h_vs_a_utility = _paired(
        h_rows,
        a_rows,
        value=lambda row: float(row["policy_utility"]),
        seed="fold-004-h-vs-a-utility",
    )
    h_vs_c_utility = _paired(
        h_rows,
        c_rows,
        value=lambda row: float(row["policy_utility"]),
        seed="fold-004-h-vs-c-utility",
    )
    ...
    for case_id in failures:
        fallback_checks.extend(
            [
                by_cell[(CONTROL_B, case_id)]["selected_action"]
                == AutonomousActionKind.NO_ACTION.value,
                by_cell[(CONTROL_C, case_id)]["selected_action"]
                == AutonomousActionKind.NO_ACTION.value,
    ...
    provider_completion = (120 - len(failures)) / 120
    gates = {
        ...
        "provider_completion_at_least_0_995": provider_completion >= 0.995,
        "fallback_is_safe_and_deterministic": fallback_rate == 1.0,
    }
    h_advances = (
        all(gates.values())
        and h_vs_a_preference["ci95"][0] >= -0.05
        and h_vs_a_utility["ci95"][0] > 0
        and aggregate[CANDIDATE_H]["mean_policy_utility"]
        >= aggregate[CONTROL_C]["mean_policy_utility"]
    )
    provisional = CANDIDATE_H if h_advances else CONTROL_A
    return {
        "status": "completed-go-deeper" if all(gates.values()) else "completed-refine",
        "decision": {
            "outcome": "go-deeper" if all(gates.values()) else "refine",
            "provisional_confirmation_candidate": provisional if all(gates.values()) else None,
            "selected_architecture_id": None,
            "rationale": (
                "Advance guarded policy-value fusion to fresh confirmation because it "
                "passed the safety gates, was non-inferior on preferred-action agreement, "
                "and established a positive paired utility improvement over A."
                if h_advances
                else "Retain deterministic A as the provisional confirmation candidate; "
                "the model-assisted successor did not establish every preregistered "
                "incremental-utility condition. No final architecture is selected from "
                "this development fold alone."
            ),
        },
        "case_count": 150,
        "condition_cell_count": 600,
        ...
        "paired_comparisons": {
            "guarded-policy-value-h-vs-deterministic-a": {
                "preferred_action_difference": h_vs_a_preference,
                "utility_difference": h_vs_a_utility,
            },
            "guarded-policy-value-h-vs-hierarchical-c": {
                "utility_difference": h_vs_c_utility,
            },
        },
        "provider_quality": {
            "planner_case_count": 120,
            "planner_failure_case_ids": sorted(failures),
            "provider_completion_rate": provider_completion,
        ...
        result.update(
            {
                ...
                "limitations": [
                    ...
                    "Preferred-action agreement is diagnostic, while the deterministic event/action envelope is authoritative.",
                    "One development successor cannot select the final release architecture without fresh confirmation.",
