...
SOURCE = ROOT / 'reports/presentation/script/cto-refined-narration.txt'
...
def run():
    ...
    (OUT / 'speaker-script-cto-refined.md').write_text('\n\n'.join(markdown) + '\n')
...
const initial=location.hash.match(/^#slide-(\d+)$/);if(initial)go(Number(initial[1]));else sync();
    ...
    (OUT / 'speaker-script-cto-refined.html').write_text(page)
    report = {
    ...
    (BUILD / 'script-checks.json').write_text(json.dumps(report, indent=2) + '\n')
    print(intro)
