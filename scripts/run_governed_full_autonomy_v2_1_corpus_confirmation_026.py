#!/usr/bin/env python3
"""Run the multi-source-corpus successor to corpus confirmation 025.
...
from scripts import build_governed_full_autonomy_v2_1_corpus_confirmation_026 as package
...
CONTEXT = shared.ActualProductEvaluationContext(
    ...
    canary_case_ids=(
        "release-corpus2-confirm-h-e1-trajectory-001-t1-v2-autonomous-seed-1",
        "release-corpus2-confirm-h-e1-long-horizon-001",
    ),
...
def validate_attempt() -> dict[str, object]:
    ...
    if instrument["method"]["request_intent_contract"] != (
        ...
        raise ValueError("confirmation 026 request-intent contract drifted")
    if instrument["execution"]["selected_evidence_gate"] != (
        "source-semantic-evidence-atom-gate-v4"
    ):
        raise ValueError("confirmation 026 must bind the v4 successor gate")
    published = instrument["dataset"]["published_sources_per_release"]
    if published < 2:
        raise ValueError(
            "confirmation 026 exists to publish a corpus; a single-source "
