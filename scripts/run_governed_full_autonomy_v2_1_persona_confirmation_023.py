#!/usr/bin/env python3
"""Run the fresh provider-backed T1-v2 confirmation 023."""
...
from scripts import build_governed_full_autonomy_v2_1_persona_confirmation_023 as package
...
CONTEXT = shared.ActualProductEvaluationContext(
    ...
    selected_grounding_architecture_id="pedagogy-aware-source-semantic-evidence-atoms-v3",
    grounding_result_expected_architecture_id="ambiguity-safe-source-semantic-evidence-atoms-v2",
    runtime_grounding_architecture_id="pedagogy-aware-source-semantic-evidence-atoms-v3",
    grounding_missing_blocker="grounding-successor-011-keep-missing",
    canary_case_ids=(
        "release-persona-confirm-h-e1-trajectory-001-t1-v2-autonomous-seed-1",
        "release-persona-confirm-h-e1-long-horizon-001",
    ),
...
def validate_attempt() -> dict[str, object]:
    ...
    if instrument["method"]["request_intent_contract"] != (
        ...
        raise ValueError("confirmation 023 request-intent contract drifted")
    if instrument["conditions"] != list(package.CONDITIONS):
        raise ValueError("confirmation 023 condition binding drifted")
    return {
        **result,
        "selected_release_candidate": "t1-v2-autonomous",
        "reactive_condition_role": "paired-grounding-diagnostic",
