...
from scripts import build_governed_full_autonomy_v2_1_corpus_confirmation_027 as package
...
def validate_attempt() -> dict[str, object]:
    ...
    if instrument["method"]["request_intent_contract"] != (
        ...
        raise ValueError("confirmation 027 request-intent contract drifted")
    if instrument["execution"]["selected_evidence_gate"] != (
        "source-semantic-evidence-atom-gate-v4"
    ):
        raise ValueError("confirmation 027 must bind the v4 successor gate")
    published = instrument["dataset"]["published_sources_per_release"]
    if published < 2:
        raise ValueError(
            "confirmation 027 exists to publish a corpus; a single-source "
