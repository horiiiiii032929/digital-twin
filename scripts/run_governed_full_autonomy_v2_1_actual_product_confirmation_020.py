#!/usr/bin/env python3
"""Run the fresh set-valued, minimal-schema release confirmation 020."""
...
from scripts import (
    build_governed_full_autonomy_v2_1_actual_product_confirmation_020 as package,
)
from scripts import (
...
CONTEXT = shared.ActualProductEvaluationContext(
    ...
    canary_case_ids=(
        "release-final-h-e1-trajectory-001-t1-v2-reactive-seed-1",
        "release-final-h-e1-long-horizon-001",
    ),
...
def validate_attempt() -> dict[str, object]:
    ...
    if instrument["method"]["reactive_provider_contract"] != (
        "minimal-reactive-intent-proposal-v3"
    ):
        raise ValueError("confirmation 020 provider contract drifted")
    return {
        **result,
