#!/usr/bin/env python3
"""Bind the call-bounded concurrent actual-product autonomy successor 008."""
...
INSTRUMENT_ID = "governed-full-autonomy-v2-1-actual-product-evaluation-008"
INSTRUMENT = ROOT / (
    "research/05_evaluation/instruments/"
    "governed_full_autonomy_v2_1_actual_product_evaluation_008.json"
)
...
def validate() -> dict[str, Any]:
    ...
    if instrument.get("successor_after") != predecessor.INSTRUMENT_ID:
        raise ValueError("predecessor binding drifted")
    correction = instrument.get("execution_correction", {})
    if correction.get("version") != "projected-call-cap-and-concurrency-v1":
        raise ValueError("execution correction contract drifted")
    if correction.get("cases_changed") is not False:
        raise ValueError("execution correction changed evaluation cases")
    if correction.get("gold_changed") is not False:
        raise ValueError("execution correction changed hidden gold")
    if correction.get("gates_changed") is not False:
        raise ValueError("execution correction changed hard gates")
    if authority["maximum_provider_calls"] != 10000:
        raise ValueError("provider-call safety ceiling drifted")
    if instrument["execution"].get("maximum_concurrency") != 8:
        raise ValueError("bounded concurrency drifted")
    ...
    return {
        ...
        "status": (
            "passed-frozen-pending-execution" if authorized else "passed-terminal"
        ),
