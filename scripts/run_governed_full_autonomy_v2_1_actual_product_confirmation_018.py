#!/usr/bin/env python3
"""Run the fresh provider-backed H+E1/V3 actual-product confirmation 018."""
...
from scripts import (
    build_governed_full_autonomy_v2_1_actual_product_confirmation_018 as package,
)
from scripts import (
...
def validate_attempt() -> dict[str, object]:
    ...
    if execution.get("direct_transport_identity_canary_required") is not True:
        raise ValueError("confirmation 018 direct transport canary drifted")
    if execution.get("product_route_canary_case_ids") != list(
        CONTEXT.canary_case_ids
    ):
        raise ValueError("confirmation 018 product-route canary drifted")
    return {
        **result,
