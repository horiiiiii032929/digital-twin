...
def _primary_pedagogy(judge: dict[str, Any]) -> tuple[dict[tuple[str, str], bool], Counter[str]]:
...
def analyze(run: dict[str, Any], judge: dict[str, Any], sensitivity: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for condition in CONDITIONS:
        ...
        summaries[condition] = {
            ...
            "professor_policy_pedagogy_proxy": _rate([row["pedagogy_success"] for row in rows]),
    ...
    gates = {
        ...
        "c3_pedagogy_proxy_at_least_0_80": c3["professor_policy_pedagogy_proxy"] >= 0.80,
    ...
    return {
        "result_id": run["run_id"], "status": "valid-complete", "decision": decision,
        ...
        "judge": {"primary": {"model": judge["model"], "digest": judge["model_digest"], "calibration_claim": "researcher-anchor-calibrated proxy only"}, "sensitivity": None if sensitivity is None else {"model": sensitivity["model"], "digest": sensitivity["model_digest"], "sample_rate": sensitivity["sample_rate"]}},
...
def render_report(result: dict[str, Any]) -> str:
    ...
    for condition in CONDITIONS:
        ...
        lines.append(f"| {condition} | {row['safe_grounded_success']:.1%} | {row['professor_policy_pedagogy_proxy']:.1%} | {row['citation_validity']:.1%} | {row['complete_evidence_at_3']:.1%} | {row['latency_p95_ms'] / 1000:.2f}s |")
...
def main() -> None:
    ...
    primary = load_json(arguments.primary_judge)
