...
const root=process.cwd(),build=root+'/reports/generated/cto-notes-build';
await fs.mkdir(build,{recursive:true});
const source=root+'/reports/presentation/deck/digital-twin-presentation-cto-refined.pptx';
const p=await PresentationFile.importPptx(await FileBlob.load(source));
const manifest=JSON.parse(await fs.readFile(root+'/reports/generated/cto-refined-build/deck-content.json','utf8'));
const scripts=(await fs.readFile(root+'/reports/presentation/script/cto-refined-narration.txt','utf8')).trim().split('\n===\n');
