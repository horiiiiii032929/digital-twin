#!/usr/bin/env python3
"""Run the sole harness-corrective attempt for confirmation 016."""
...
INSTRUMENT_ID = "governed-full-autonomy-v2-1-actual-product-confirmation-017"
INSTRUMENT = ROOT / (
    "research/05_evaluation/instruments/"
    "governed_full_autonomy_v2_1_actual_product_confirmation_017.json"
)
...
def validate_attempt() -> dict[str, object]:
    ...
    public = package.public_payload()
    hidden = package.hidden_gold_payload()
    if instrument["instrument_id"] != INSTRUMENT_ID:
        raise ValueError("confirmation 017 identity drifted")
    if instrument["dataset"]["dataset_id"] != package.INSTRUMENT_ID:
        raise ValueError("confirmation 017 dataset identity drifted")
    if public["content_sha256"] != instrument["dataset"]["public_sha256"]:
        raise ValueError("confirmation 017 public package drifted")
    if hidden["content_sha256"] != instrument["dataset"]["hidden_gold_sha256"]:
        raise ValueError("confirmation 017 hidden package drifted")
    ...
    return {
        ...
        "case_count": len(package.build_contract()),
...
BUILDER = SimpleNamespace(
    ...
    DAY=package.DAY,
    build_contract=package.build_contract,
    public_payload=package.public_payload,
    hidden_gold_payload=package.hidden_gold_payload,
    validate=validate_attempt,
)
