#!/usr/bin/env python3
"""Run the fresh structured-request-intent release confirmation 021."""
...
from scripts import (
    build_governed_full_autonomy_v2_1_actual_product_confirmation_021 as package,
)
...
CONTEXT = shared.ActualProductEvaluationContext(
    ...
    canary_case_ids=(
        "release-decision-h-e1-trajectory-001-t1-v2-reactive-seed-1",
        "release-decision-h-e1-long-horizon-001",
    ),
...
def validate_attempt() -> dict[str, object]:
    ...
    if instrument["method"]["request_intent_contract"] != (
        ...
        raise ValueError("confirmation 021 request-intent contract drifted")
    return {
        **result,
        "action_gold_contract": "set-valued-valid-actions-v2",
        "reactive_provider_contract": "minimal-reactive-intent-proposal-v3",
