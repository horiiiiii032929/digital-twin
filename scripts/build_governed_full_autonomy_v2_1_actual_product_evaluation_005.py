#!/usr/bin/env python3
"""Bind the user-requested final transient-failure retry as attempt 005."""
...
INSTRUMENT_ID = "governed-full-autonomy-v2-1-actual-product-evaluation-005"
INSTRUMENT = ROOT / (
    "research/05_evaluation/instruments/"
    "governed_full_autonomy_v2_1_actual_product_evaluation_005.json"
)
...
def validate() -> dict[str, Any]:
    ...
    if instrument.get("instrument_id") != INSTRUMENT_ID:
        raise ValueError("actual-product retry identity drifted")
    if instrument.get("status") not in {
        ...
        raise ValueError("actual-product retry status is invalid")
    ...
    if authority["automatic_promotion"] is not False:
        raise ValueError("actual-product retry cannot auto-promote")
    if instrument.get("retries_invalid_execution") != (
        "governed-full-autonomy-v2-1-actual-product-evaluation-004"
    ):
        raise ValueError("invalid predecessor binding drifted")
    retry = instrument.get("execution_retry", {})
    if retry.get("changed_surface") != []:
        raise ValueError("retry changed the evaluated surface")
    if retry.get("method_changed") is not False or retry.get("gates_changed") is not False:
        raise ValueError("retry changed method or gates")
    ...
    return {
        ...
        "status": "passed-frozen-pending-execution" if authorized else "passed-terminal",
