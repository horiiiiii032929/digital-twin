#!/usr/bin/env python3
"""Build the fresh H+E1 actual-product autonomy confirmation package."""
...
from scripts import (
    build_governed_full_autonomy_v2_1_actual_product_confirmation_013 as predecessor,
)
from scripts import (
...
INSTRUMENT_ID = "governed-full-autonomy-v2-1-actual-product-confirmation-014"
INSTRUMENT = ROOT / (
    "research/05_evaluation/instruments/"
    "governed_full_autonomy_v2_1_actual_product_confirmation_014.json"
)
...
def fresh_source_number(case_id: str) -> int:
    """Map every scenario to one of 50 source identities unused by 012/013."""

    return 201 + ((source_base._base_number(case_id) - 1) % 50)  # noqa: SLF001


def source_fixture_for_case(case_id: str) -> dict[str, str]:
    number = fresh_source_number(case_id)
    return {
        "source_id": f"synthetic-h-e1-source-{number:03d}",
...
def _message(case_id: str, turn_kind: str) -> str:
    ...
    return {
        "direct": (
            "According to the approved source, what does bounded recovery protocol "
...
def build_contract():
    ...
    for condition, old_case, old_gold in predecessor.build_contract():
        case_id = old_case.case_id.replace("release-final-", "release-h-e1-", 1)
        course_id = old_case.course_id.replace("release-final-", "release-h-e1-", 1)
        release_id = old_case.release_id.replace("release-final-", "release-h-e1-", 1)
        learner_id = old_case.learner_id.replace("release-final-", "release-h-e1-", 1)
        events = []
...
def public_payload() -> dict[str, Any]:
    payload = {
        ...
        "case_count": 820,
        "source_family_range": [201, 250],
        "source_disjoint_from_confirmations_012_013": True,
        "rows": [
...
def validate() -> dict[str, Any]:
    ...
    historical_sources = {
        ...
        predecessor.predecessor.source_fixture_for_case(case.case_id)["source_id"]
        for _condition, case, _gold in predecessor.predecessor.build_contract()
    }
    if current_sources & historical_sources or len(current_sources) != 50:
        raise ValueError("confirmation 014 source identities are not fresh")
