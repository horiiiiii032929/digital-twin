#!/usr/bin/env python3
"""Bind the prompt-schema-aligned actual-product autonomy successor 007."""
...
INSTRUMENT_ID = "governed-full-autonomy-v2-1-actual-product-evaluation-007"
INSTRUMENT = ROOT / (
    "research/05_evaluation/instruments/"
    "governed_full_autonomy_v2_1_actual_product_evaluation_007.json"
)
...
def validate() -> dict[str, Any]:
    ...
    correction = instrument.get("runtime_correction", {})
    if correction.get("version") != "bounded-prompt-schema-alignment-v1":
        raise ValueError("runtime correction contract drifted")
    if correction.get("cases_changed") is not False:
        raise ValueError("runtime correction changed evaluation cases")
    if correction.get("gold_changed") is not False:
        raise ValueError("runtime correction changed hidden gold")
    if correction.get("gates_changed") is not False:
        raise ValueError("runtime correction changed hard gates")
