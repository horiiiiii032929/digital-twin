...
```json
{
  ...
  "scripts": {
    ...
    "dev:api": "uv run uvicorn services.api.app.main:app --reload --port 8000",
    "test": "uv run pytest tests",
    "test:api": "uv run pytest tests",
...
- `uv sync --dev`: install the Python API and test dependencies.
...
Expected: package installs successfully and `uv run pytest tests` can run even if no new tests are present yet.
...
```bash
uv run pytest tests/digital_twin/test_tutor_policy.py -v
```
...
```bash
uv run pytest tests/digital_twin/test_llm.py -v
```
...
```bash
uv run pytest tests/digital_twin/test_onboarding_workflow.py -v
```
...
```bash
uv run pytest tests/api/test_onboarding_api.py -v
```
...
```bash
uv run pytest tests/digital_twin tests/api -v
```
