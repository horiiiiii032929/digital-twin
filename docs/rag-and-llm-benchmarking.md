...
| Retrieval | Did the context window contain all evidence needed for the question? | no prohibited or superseded source returned | gold-evidence Recall@3, complete-evidence success@3, graded nDCG@3 | Recall@5, Precision@3/5, MRR, category slices, latency and index burden | M2 hybrid retrieval is selected in the experimental profile with BM25 v1 as rollback; the invalid professor-fidelity C3 run does not add evidence because its chunker corpus drifted |
