...
## Active local release qualification
...
The Project should therefore show #180 as `In Progress / Keep` while local
Docker/HTTPS and browser qualification run. Governed T1-v2.1 is selected only
for the exact hash-bound local R1 profile; T0 remains rollback. #24, hosted
deployment, real-professor fidelity, real-student usability, and learning-
outcome claims remain open and are not implied by this local selection.
