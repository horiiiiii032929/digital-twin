...
| Generator and prompt | Refine current V4 Pro candidate; historical V4 Flash selection preserved | V4 Pro completed the unchanged 48-case public-synthetic development run at one exact fingerprint: original 46/48 deterministic all-check passes; first Qwen review copied a template and Qwen v2 failed the missing-citation sensitivity probe; deterministic fallback remains | Frozen no-model clarification-action correction, a repaired candidate for the remaining real failure, calibrated semantic citation review, and any prospective profile replacement |
...
1. Run the frozen no-model clarification-action analysis correction. Preserve
   the original 46/48 result and verify the prediction that exactly one
   instrumentation false negative changes, leaving one real ambiguity failure;
   no generator held-out is authorized.
