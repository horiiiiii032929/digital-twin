...
2. Preserve closed #87 and completed #110 as engineering evidence that the
   ...
   are built, but the new pre-spend audit blocks the 500-case run until complete
   source statements and exact structured regions replace the defective
   references and the package passes the same audit again. Only then should a
   provider binding and separate paid authorization be requested. Panel
