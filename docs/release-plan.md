...
| Factual quality | The actual T0 product must retrieve and answer without receiving gold answers, claims, evidence, or citations | AFQC-044 resolves the failed provider-construction loop with a provider-free deterministic package: 500 public development cases, separate hidden gold, 400 answerable/100 boundary composition, a fixed 100-case control, exact source-range lineage, and zero duplicates or answer leakage. AFQC-045 preserves that structural pass but blocks product use after 227/400 answerable rows triggered high-risk reference-quality diagnostics and a 12-case Codex-assisted audit confirmed material defects. Only 16/35 code-labelled and 2/3 table-labelled development windows contain complete detected regions, so a prospective source-plan successor is required. No T0 product result exists yet; the sealed 10,000-case run remains unauthorized | [#127](https://github.com/horiiiiii032929/digital-twin/issues/127), with [#110](https://github.com/horiiiiii032929/digital-twin/issues/110) preserved as engineering history |
...
2. Preserve closed #87 and completed #110 as engineering evidence that the
   ...
   #127. The feasible course allocation and deterministic development package
   are built, but the new pre-spend audit blocks the 500-case run until a
   prospective 100-cluster source plan provides complete source statements and
   exact structured regions, and the corrected package passes the same audit
   again. Only then should a provider binding and separate paid authorization
   be requested. Panel
