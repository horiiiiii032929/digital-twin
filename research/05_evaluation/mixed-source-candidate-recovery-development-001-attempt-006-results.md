# Evaluation result: mixed-source-candidate-recovery-development-001-attempt-006

## Identity and decision

**Keep integration coverage only.** Recorded on 2026-09-06, revision `e441193c7fa1260ed327ebf43c59483f85817be8`,
dirty; source hashes unchanged: True. [Machine record](records/mixed-source-candidate-recovery-development-001-attempt-006.json).
The original manifest, summary and any ledgers remain under
`reports/generated/mixed-source-candidate-recovery-development-001-attempt-006/`. No attempt was overwritten.

Expanded mixed-source contract passed, including both additional trust-boundary negatives.
...
| Measure | Result |
| --- | ---: |
| Executed / passed assertions | 143 / 143 |
| Distinct assertion labels | 108 |
| Injected / external provider calls | 12 / 0 |
| External provider cost | USD 0 |
| Wall time | 2.189324s |
| Peak process RSS | 312,819,712 bytes |
...
This final expansion also rejects a cross-origin mutation without creating a course
and rejects a completed course-B source job in a course-A draft, despite common
professor ownership. There are 143 assertions with 108 distinct labels in one joined
scenario; seven student turns are not143 independent observations. A separate
routine restored-PDF crop endpoint regression checks PNG retrieval from the new
root and wrong-learner denial with zero provider calls; it is not added to these
named-run counts.
