...
from scripts import build_academic_factual_qa_semantic_target_successor as builder
...
def test_semantic_target_successor_is_fresh_balanced_and_finite(result) -> None:
...
def test_semantic_target_successor_ranges_do_not_overlap_prior_packages(result) -> None:
...
def test_semantic_target_successor_has_unique_matchable_atoms(result) -> None:
    source = result["packages"]["source"]
    cases = result["packages"]["cases"]["cases"]
...
def test_semantic_target_successor_uses_pinned_think_os_source(result) -> None:
    source = result["packages"]["source"]
    os_rows = [
        row for row in source["clusters"] if row["course_id"] == "operating-systems"
    ]
    assert len(os_rows) == 25
    assert {row["repository_commit"] for row in os_rows} == {builder.THINK_OS_COMMIT}
    assert {row["license_spdx"] for row in os_rows} == {builder.THINK_OS_LICENSE}
