# Reader-focused presentation script

33 main slides, Questions, 8 backup slides. 5118 narration words; video 4:06.

At 170 words/min plus video: 34.2 minutes, before transitions. Definitions are reading support; avoid reading them twice.
...
## 2. What this system is designed to do
...
Sources: research/06_reports/final/chapters/01-objective.tex, research/06_reports/final/chapters/02-course-workflow.tex

## 3. Delivery against the project brief
...
Terminology on this slide: Digital twin here = a course tutor configured by an instructor; fidelity to a real instructor is untested. Grounded = supported by approved course evidence. An acceptance gate is a required pass threshold.
...
## 4. What to watch in the demonstration
...
## 5. Thirty virtual days in the actual software
...
Terminology on this slide: UML = Unified Modeling Language. Lanes show responsibility; diamonds branch or merge; arrows show order.
...
Terminology on this slide: Profile = approved teaching settings. Elicitation asks for a student attempt; diagnosis identifies its specific error.
...
## 8. System architecture and comparison boundaries
...
Terminology on this slide: Release = a published version of sources and settings. Worker = a process that handles queued or scheduled work. C4 = Context, Containers, Components, Code. This view shows applications and stores, not a network deployment.
...
Terminology on this slide: A goal is a persistent support objective. “Completed” is a record state, not proof of educational mastery.
...
## 10. What the simulation executes and supplies
...
Terminology on this slide: A simulated driver supplies student inputs. A virtual clock advances elapsed time without waiting real days.
...
## 11. How to read the experimental evidence
...
## 15. Explicit required facts helped; extra ranking did not
...
Terminology on this slide: Typed target = an explicit required fact or set; cardinality = the required number of items. p95 latency = 95% of measured requests finished within this time. Lower is faster.

Sources: research/05_evaluation/course-digital-twin-whole-system-architecture-round-2-001-results.md

## 16. Retrieval did not ensure complete answers
...
Terminology on this slide: BM25 = keyword-based ranking; Qwen3 hybrid adds semantic retrieval. The gate filters acceptable evidence. All evidence@3 = every required fact retrieved in the top 3; grounded = complete, supported answer. Boundary action = correct refusal/clarification when required. Source-disjoint = test sources differ from development.

Sources: research/05_evaluation/final-cross-method-factual-confirmation-001-results.md

## 17. Visual retrieval and grounded answers
...
Terminology on this slide: Visual retrieval selects a relevant figure or region. A grounded answer must also use its evidence correctly. “Omni” labels the tested image-capable candidate. These are separate datasets, not a before/after trend.

Sources: research/06_reports/final/chapters/04-comparisons.tex

## 18. Planner alternatives and the reject-only failure
...
Terminology on this slide: Planner = chooses the next action. Lookahead estimates its consequences; the verifier accepts or rejects it. A/B/C/C+V are local design labels, not standard algorithm names. V denotes the added verifier.
...
## 19. Guarded replacement retains the baseline
...
This preserves a useful available move instead of allowing a reject-only stage to remove it without replacement. In the fresh fourth fold, all four methods produced valid actions. H had utility zero point seven eight eight zero compared with zero point seven eight six zero for A. The other methods did not dominate all metrics.

In the separate one-thousand-context confirmation, H again had a small positive paired utility improvement. The means were zero point eight zero zero two for H and zero point seven nine five four for A. Preferred-label agreement fell from seventy-four to seventy-three percent. The result supports conditional progression, not improvement on every measure.

There is still an important unresolved question. The analytic ranking, fallback and model proposal work together. This comparison does not isolate the value added by the language model itself. An analytic-only ablation would be needed. Also, this utility comes from a synthetic instrument and must not be presented as a measured gain in student learning. The default learner proxy remains another limitation in the inputs to this design.

Terminology on this slide: H = guarded replacement. Utility = study-specific simulated action value, not measured learning gain. Preferred = matches the reference action; valid = obeys the permitted action/transition rules. Higher utility is better.
...
## 20. Learner models and intervention timing
...
Terminology on this slide: All estimators read assessed correctness and time, not raw text. The timing policy chooses whether to intervene.
...
## 21. BKT example: updating one concept after an answer
...
## 22. How learner estimation and timing were evaluated
...
Terminology on this slide: Hidden mastery is available only to evaluation, not to the tested policy (except the oracle bound). Bootstrap = resample learners to estimate uncertainty. These intervals do not establish real-student benefits. Seed = reproducible random sequence; log loss penalizes confident wrong predictions.

Sources: research/05_evaluation/successor-learner-timing-simulation-001-results.md, src/digital_twin/student/learner_estimators.py, src/digital_twin/student/intervention_policies.py

## 23. Learner estimation and delivery timing

The table separates the estimator from timing. With the count estimator fixed, conditional timing roughly halved the messages, but final simulated mastery also fell. Value timing retained more practice and reduced waste. BKT with value timing had lower hidden-state error and higher simulated final mastery than the count/constant control. However, the matched BKT-versus-count comparison under constant timing did not establish an improvement in next-answer Brier score: its interval includes zero. A better match to hidden simulator state is therefore not equivalent to better prediction of observed answers. All results depend on the simulated learning and receptivity rules. The full eleven-condition grid remains available in backup. BKT is an experimental candidate, not the default planner-input implementation. That integration distinction is the next finding.

Terminology on this slide: MSE = mean squared error against hidden simulator mastery (lower is better); messages = mean per learner. Waste = sent while mastery ≥ 0.85 or the learner is unreceptive. Mastery = hidden day-30 simulator state.

Sources: research/05_evaluation/successor-learner-timing-simulation-001-results.md

## 24. The default planner still receives a delivery proxy
...
Terminology on this slide: Proxy = an indirect substitute. Here the count measures delivered actions, not correct student answers.

Sources: research/06_reports/final/chapters/03-runtime-design.tex, reports/presentation/design-comparisons.md

## 25. Structured wording retained critical semantic errors
...
Terminology on this slide: Adequate/defective = initially acceptable/flawed drafts. Critical = a serious change to the source’s meaning.
...
## 26. Completion used evidence from the wrong objective
...
Terminology on this slide: Objective = the goal being pursued; target concept = the specific subject whose assessed evidence may satisfy it.
...
## 27. The correction fixed completion scope
...
Terminology on this slide: Supported = meets the target-concept completion rule. A history = one simulated learner’s 30-day interaction. Autonomous slice = histories where the system may initiate support; it is a subset of the full audit.

Sources: research/05_evaluation/goal-completion-scope-development-001-results.md

## 28. Authority is checked when saving a turn
...
Terminology on this slide: Commit = save accepted effects. Revision = state version; a mismatch means the generated result may be stale.

Sources: research/06_reports/final/chapters/03-runtime-design.tex

## 29. A retry recognizes the saved in-app delivery
...
Terminology on this slide: Stable key = the same identifier across retries. Lease = temporary worker ownership; neither grants permission.

Sources: research/06_reports/final/chapters/03-runtime-design.tex

## 30. The integrated trial ran, but support stayed generic
...
Terminology on this slide: Safe graph failure = the tutor workflow stopped safely instead of completing a response. A synthetic reply is produced by the simulation; it is not evidence that a real student found the message useful.

Sources: research/05_evaluation/final-profile-operational-dialogue-development-001-full-live-001-results.md

## 31. Evaluation corrections limit the surviving claims
...
Terminology on this slide: Independent = answers were not visible during authoring. Diagnostic results help explain faults, not select a winner.
...
## 32. Next comparison: the planner-input adapter
...
Terminology on this slide: Adapter = code translating saved state into planner input. Hold other components fixed to isolate its effect.

Sources: research/06_reports/final/chapters/03-runtime-design.tex, research/05_evaluation/goal-completion-scope-development-001-results.md

## 33. Deliverable, findings and acceptance gaps

The deliverable is a working research prototype with inspectable decisions and persistent support. The video showed the software operating across multiple synthetic users and thirty virtual days. The studies explain why I retained some components and did not promote others.

The strongest engineering findings concern specific boundaries. Typed targets improved a development evidence contract. A reject-only verifier could lose an available useful move. Guarded replacement preserved the control. Exact objective scope removed unsupported completion in the matched study.

Those findings do not add up to proven teaching effectiveness. Factual acceptance, instructor fidelity and the usefulness of intervention remain incomplete. The next comparisons must address those gaps while keeping the current control and making failure visible.

I would therefore describe the project as an implemented and evaluated research prototype with bounded engineering findings. It is ready for a focused discussion of the next acceptance decision, rather than a claim that autonomous teaching quality has already been solved.

Terminology on this slide: Acceptance gap = a project requirement for which sufficient evidence has not yet been demonstrated.
...
Terminology on this slide: C4 = Context, Containers, Components, Code. This context view shows users and the system boundary.
...
Terminology on this slide: Multiplicity: 1 = exactly one; 0..* = zero or more. A release links conversations and goals to approved material.
...
Terminology on this slide: Preflight = checks before publication. A transaction saves related changes together or rolls them back.
...
Terminology on this slide: C selects an action using proposal + lookahead; V checks that choice. Rejection returns no replacement.
...
Terminology on this slide: Count = smoothed correct-attempt fraction; BKT = Bayesian Knowledge Tracing; PFA = Performance Factors Analysis. Oracle = knows hidden need (upper bound); never = sends nothing. All conditions use the same eligibility rules.
...
Terminology on this slide: Luna, Terra and Mini name the model allocations used in this study. Final wording includes fallback responses. An effect interval including zero does not establish an improvement. Utility is a simulated decision score.
...
Terminology on this slide: Keep = retain; Refine = correct weaknesses; Go Deeper = investigate before release; Drop = reject the candidate.
...
Terminology on this slide: A fold is a development/test subset. A correction narrows what an earlier run can validly establish.
